...
export default async function dashboardRoutes(app) {
  ...
  app.get('/', opts, async (request) => {
    ...
    const [
      totalProcessos, processosAtivos, totalClientes, prazosProximos,
      receitaMes, despesaMes, ultimosAndamentos, proximosEventos
    ] = await Promise.all([
      ...
      prisma.evento.count({
        where: { escritorioId, tipo: 'PRAZO', concluido: false, dataInicio: { gte: hoje, lte: proximos7dias } }
      }),
