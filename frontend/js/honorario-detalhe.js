...
async function init() {
  ...
  const venc = h.dataVencimento ? new Date(h.dataVencimento.replace('Z','')).toLocaleDateString('pt-BR') : '—';
  ...
  document.getElementById('hero-stats').innerHTML = `
    ...
    <div class="h-stat"><div class="h-stat__label">Vencimento</div><div class="h-stat__value${h.status === 'Em atraso' || h.status === 'ATRASADO' ? ' h-stat__value--urgent' : ''}">${venc}</div></div>
  ...
  const isPago = h.status === 'Pago' || h.status === 'PAGO';
  if (isPago) {
    btnPago.disabled = true;
    btnPago.textContent = 'Já pago';
    btnPago.style.opacity = '0.5';
  } else {
    btnPago.onclick = () => confirmarPagamento(h);
  }

  // ---------- BOTÃO EDITAR ----------
  const btnEditar = document.getElementById('btn-editar');
  if (btnEditar) btnEditar.onclick = () => abrirModalEdicao(h);

  if (typeof Notifications !== 'undefined') Notifications.init('btn-notificacoes');
}

// ---------- CONFIRMAÇÃO DE PAGAMENTO ----------
// FIX: modal próprio com botão "Confirmar" em vez do Confirm genérico (que só tinha "Excluir")
// FIX: endpoint corrigido de /financeiro/${id}/pagar para /financeiro/${id} com body { status: 'PAGO' }
function confirmarPagamento(h) {
    ...
    <div style="background:#fff;border-radius:16px;padding:28px 28px 20px;width:360px;max-width:90vw;box-shadow:0 8px 32px rgba(0,0,0,0.18)">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">
        ...
        <div>
          ...
          <div style="font-size:12px;color:#9a9a94;margin-top:2px">Confirma o recebimento de R$ ${Number(h.valor).toLocaleString('pt-BR')}?</div>
        </div>
      </div>
  ...
  document.getElementById('confirm-pagar-ok').addEventListener('click', async () => {
    overlay.remove();
    try {
      await Api.patch(`/financeiro/${honId}`, { status: 'PAGO' });
...
function abrirModalEdicao(h) {
  ...
  const statusOpts = ['PENDENTE','PAGO','ATRASADO','Parcelado'].map(s =>
    `<option value="${s}" ${h.status === s ? 'selected' : ''}>${sMap[s]?.label || s}</option>`
  ).join('');

  const vencValue = h.dataVencimento
    ? new Date(h.dataVencimento.replace('Z','')).toISOString().split('T')[0]
    : '';
      ...
      <div style="padding:20px 24px;display:flex;flex-direction:column;gap:14px">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div style="display:flex;flex-direction:column;gap:5px">
            ...
            <input id="edit-valor" type="number" value="${h.valor}" min="0" step="0.01"
        ...
        <div style="display:flex;flex-direction:column;gap:5px">
          ...
          <input id="edit-venc" type="date" value="${vencValue}"
            style="padding:8px 12px;border:0.5px solid rgba(0,0,0,0.14);border-radius:8px;font-size:13px;color:#1c1c1a;font-family:'DM Sans',sans-serif" />
        </div>
      </div>
  ...
  document.getElementById('modal-edicao-save').addEventListener('click', async () => {
    ...
    try {
      await Api.patch(`/financeiro/${honId}`, {
        valor,
        status,
...
function renderHistorico(h, venc) {
  const historico = [
    ...
    ...(h.status === 'Pago' || h.status === 'PAGO' ? [{ evento: 'Pagamento recebido', data: 'Registrado', obs: `R$ ${Number(h.valor).toLocaleString('pt-BR')} confirmado` }] : []),
    ...(h.status === 'Em atraso' || h.status === 'ATRASADO' ? [{ evento: 'Vencimento em atraso', data: venc, obs: 'Pagamento não identificado até a data de vencimento.' }] : []),
  ];
  document.getElementById('pane-historico').innerHTML = `<div class="timeline">
    ${historico.map((item, i) => `<div class="tl-item"><div class="tl-spine">
      <div class="tl-dot" style="background:${i === 0 ? '#3d7a52' : i === historico.length - 1 && (h.status === 'Em atraso' || h.status === 'ATRASADO') ? '#c0392b' : '#2563a8'}"></div>
